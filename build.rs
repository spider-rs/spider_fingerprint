...
fn assets_dir(manifest: &Path) -> PathBuf {
...
fn pick_referrers_input(manifest: &Path) -> (PathBuf, usize) {
    let assets = assets_dir(manifest);

    let expand = std::env::var("SPIDER_FP_EXPAND_REFERRERS").ok().as_deref() == Some("1");

    if expand {
        let p = assets.join("merged_referrers.txt");
        if p.exists() {
            // We’re not forcing 1M here; you can cap in REFERRERS_TRIM_TO_1M,
            // but generation will cap to the chosen usize (below) if you want.
            // If you want “use all”, bump the cap.
            return (p, 1_000_000);
        }
        eprintln!(
            "referrers: SPIDER_FP_EXPAND_REFERRERS=1 but {} missing; falling back to 20k",
            p.display()
        );
    }

    (assets.join("referrers_20k_urls.txt"), 20_000)
}
